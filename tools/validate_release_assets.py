"""Static release-asset validation for the X-CLIP zero-shot video classification DIMER pipeline.
...
EXPECTED_OUTPUTS = (
    ...
    "outputs/xclip_video_classification_rankings.csv",
    "outputs/xclip_video_classification_contact_sheet.png",
)
...
CODE_MARKERS = (
    ...
    "sample_frames(frames_from_animation(animation))",
    "hashlib.sha256(b''.join(np.asarray(frame.convert('RGB')).tobytes() for frame in clip)).hexdigest()",
    "result['top1']",
...
MARKDOWN_MARKERS = (
    ...
    "The label set is a **caller-owned request parameter**",
    ...
    "`sample-sanity`",
    "**The label set is part of the request**",
    "Video decoding from container formats",
)
